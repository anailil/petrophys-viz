...
def plot_well_curve(plot, curve_type, curve_depth, color, x_label, y_label, graphlabel='', linewidth=0.5, label_position='top', grid=True, grid_color='g', alpha=0.3):
    ...
    plt.setp(plot.get_xticklabels()[1::2], visible=False)  # Hide every second tick-label
    remove_last(plot)  # remove last value of x-ticks, see function defined in first cell

#track1: Gamma Ray
plot_well_curve(ax1, lasfile['GR'], lasfile['DEPT'], 'c', 'GR (API)', 'DEPTH (m)')
